...
public partial class HistoricalObservationsAdaptive : ContentView
{
    readonly IDispatcherTimer _autoScrollTimer;
    bool _autoScrollEnabled;
    int _autoScrollIndex;

    public HistoricalObservationsAdaptive(HistoricalObservationsViewModel viewModel)
    {
        ...
        Unloaded += (_, _) =>
        {
            ...
            {
                try { Debug.WriteLine($"HistoricalObservationsAdaptive: failed to stop auto-scroll timer. {ex.Message}"); } catch { }
            }
        };
    }
    ...
    void AutoScrollTick()
    {
        try
        {
            ...
            if (BindingContext is not HistoricalObservationsViewModel vm)
                return;
        ...
        {
            try { Debug.WriteLine($"HistoricalObservationsAdaptive: auto-scroll failed. {ex.Message}"); } catch { }
        }
    }
}
