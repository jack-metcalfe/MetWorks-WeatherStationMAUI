...
public sealed class LightningRollupRepository : ILightningRollupRepository
{
    ISqliteDatabase? _sqliteDatabase;
    IInstanceIdentifier? _instanceIdentifier;

    public LightningRollupRepository()
    {
    }
    ...
    ISqliteDatabase GetInitializedSqliteDatabase() =>
        _sqliteDatabase ?? throw new InvalidOperationException($"{nameof(LightningRollupRepository)} has not been initialized.");

    IInstanceIdentifier GetInitializedInstanceIdentifier() =>
        _instanceIdentifier ?? throw new InvalidOperationException($"{nameof(LightningRollupRepository)} has not been initialized.");

    public Task RollupDayAsync(int maxBucketsPerRun, CancellationToken cancellationToken)
    {
        return RollupAsync(
            rollupTableName: "lightning_rollup_1d",
        ...
        CancellationToken cancellationToken)
    {
        ...
        var watermark = await watermarkStore.TryGetWatermarkAsync(
            session,
            LightningRollupSql.SourceTableName,
            bucketWidthSeconds,
        ...
        var sql = LightningRollupSql.BuildUpsertRollupSql(rollupTableName, bucketWidthSeconds);
        ...
        CancellationToken cancellationToken)
    {
        return session.ScalarAsync<long?>(
            "SELECT MIN(device_received_utc_timestamp_epoch) FROM lightning WHERE installation_id = $installation_id;",
            [new DbParam("$installation_id", installationId)],
            cancellationToken);
    }
    ...
    {
        return session.ScalarAsync<long?>(
            "SELECT MAX(device_received_utc_timestamp_epoch) FROM lightning WHERE installation_id = $installation_id;",
            [new DbParam("$installation_id", installationId)],
            cancellationToken);
    }
}
