...
public sealed class LightningStreamShipper : ServiceBase
{
    ...
    )
    {
        ...
        {
            ILogger.Warning("LightningStreamShipper has no SQLite connection configured; shipper will not run.");
            try { MarkReady(); } catch { }
            return true;
        }
    ...
    async Task ShipLoopAsync(TimeSpan interval, CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            ...
            {
                ILogger.Warning($"LightningStreamShipper: HTTP failure: {ex.Message}");
            }
            catch (SqliteException ex)
            {
                ILogger.Warning($"LightningStreamShipper: SQLite failure: {ex.Message} (code={ex.SqliteErrorCode})");
            }
            catch (InvalidOperationException ex)
            {
                ILogger.Warning($"LightningStreamShipper: failure: {ex.Message}");
            }
        }
    }
