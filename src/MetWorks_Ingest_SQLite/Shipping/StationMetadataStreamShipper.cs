...
public sealed class StationMetadataStreamShipper : ServiceBase
{
    ...
    )
    {
        ...
        {
            ILogger.Warning("StationMetadataStreamShipper has no SQLite connection configured; shipper will not run.");
            try { MarkReady(); } catch { }
            return true;
        }
    ...
    async Task ShipLoopAsync(TimeSpan interval, CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            ...
            {
                ILogger.Warning($"StationMetadataStreamShipper: SQLite failure: {ex.Message} (code={ex.SqliteErrorCode})");
            }
    ...
    async Task ShipOnceAsync(CancellationToken token)
    {
        ...
        await StationMetadataStreamShipping.ShipOnceAsync(
            conn,
