...
public sealed class ObservationStreamShipper : ServiceBase
{
    ...
    const string DdlScript = "Ingest/SQLite/observation.sql";
    ...
    )
    {
        ...
        {
            ILogger.Warning("ObservationStreamShipper has no SQLite connection configured; shipper will not run.");
            try { MarkReady(); } catch { }
            return true;
        }
    ...
    async Task ShipLoopAsync(TimeSpan interval, CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            ...
            {
                ILogger.Warning($"ObservationStreamShipper: SQLite failure: {ex.Message} (code={ex.SqliteErrorCode})");
            }
