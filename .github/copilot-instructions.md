...
- Address project dependency hygiene to avoid cycles when centralizing persistence config (e.g., `MetWorks_Common -> MetWorks_Common_Settings -> MetWorks_Resource_Store -> MetWorks_Common`); consider refactoring/interfaces as needed. **Avoid breaking reference cycles by pulling persistence upward or adding interfaces; instead, move shared contracts downward and enforce layering.**
...
- SQLite `TEXT` columns are case-sensitive by default. Apply `COLLATE NOCASE` in the schema DDL (not at query time) for any column that stores identifiers compared case-insensitively (e.g., `installation_id` in `shipper_state`). SQLite has no native GUID/UUID datatype; GUIDs are stored as `TEXT` (COMB GUIDs) or `BLOB`.
...
- Schema readiness ownership: the repository that owns a table is responsible for lazily ensuring its schema exists via a semaphore-guarded one-time `EnsureSchemaAsync` call (e.g., `StreamShippingRepository` for `shipper_state`, `StationMetadataRepository` for `station_metadata`). Higher-level shippers, ingestors, and orchestrators must NOT own or call schema readiness for tables they do not define; doing so leaks persistence implementation details and causes early callers (e.g., `MetricsSamplerService`) to hit missing-table errors before the schema is created. The repository accepts the matching `IDatabaseReadiness` interface as an `InitializeAsync` parameter and delegates to it inside `EnsureSchemaAsync`.
